...
exports.getName = async( accessToken, id) =>{
    // console.log("Playlist id thingy: "+id)
    var response = await fetch(`https://api.spotify.com/v1/playlists/${id}`, {
        headers: {
            Authorization: 'Bearer ' + accessToken
        }
    })
...
exports.getPlaylists = async (accessToken, userID, playlists=[]) => {
    ...
    localStorage.setItem(`${userID}-sPlaylistAll`, JSON.stringify(playlists))
    localStorage.setItem(`${userID}-sPlaylist`, JSON.stringify(cleanData))
    var currPlaylist = cleanData[Object.keys(cleanData)[3]]["id"]
    localStorage.setItem(`${userID}-sPlaylistName`,cleanData[Object.keys(cleanData)[3]]["name"])
    this.getPlaylistSongs(userID,accessToken,currPlaylist)
    return cleanData
}

exports.getPlaylistSongs = async (userID,accessToken, playlistID, trackList = []) =>{
    var cleanData = {}
    var songlist = []
    var songData = await getChunkSongs(accessToken, playlistID, 0, null ,songlist)
    
    localStorage.setItem(`${userID}-sSongsPlaylist`, JSON.stringify(songData))
...
async function getChunkSongs( accessToken, playlistID,offset = 0, url=null, trackList = []){
    ...
    if (url == null) {
        ...
        var response = await fetch(`https://api.spotify.com/v1/playlists/${playlistID}/tracks?limit=50&fields=items(track(name,duration_ms,artists(name),album(images))),next`, {
            headers: {
                Authorization: 'Bearer ' + accessToken
            }
        })
