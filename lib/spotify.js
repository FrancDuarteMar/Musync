...
exports.getPlaylistSongs = async (accessToken, playlistID, trackList = []) =>{
    ...
    // localStorage.setItem('sSongName',)
    localStorage.setItem('sSongList',JSON.stringify(songlist))
    // for (const elem of songlist){
        // console.log(elem['track']["name"]) //get track name
    //     console.log(elem["track"]["artists"][0]["name"]) // get first artist name 
    // }\
    
    // for (const elem of JSON.parse(localStorage.getItem("sSongList"))){
    //     console.log(elem['track']["name"]) //get track name

    // }
    
    // console.log("Song list thing: "+ localStorage.getItem("sSongList"))
